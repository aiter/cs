...
struct TCPserver * tcp_server_init(struct event_loop *eventLoop, struct acceptor *acceptor, connection_completed_call_back connectionCompletedCallBack, message_call_back messageCallBack, write_completed_call_back writeCompletedCallBack, connection_closed_call_back connectionClosedCallBack, int threadNum) {
...
int handle_connection_established(void *data) {
	...
	// choosae event loop from the thread pool
...
void tcp_server_start(struct TCPserver *tcpServer) {
	...
	//acceptor主线程，同时把tcpServer作为参数传递给channel对象
